    ...
    */
    ...
    this.initCarousel = function (carousel, tabletNumb, desktopNumb) {
        var visibleSlides = carousel.querySelectorAll('.c-triage-result-card:not(.hide), .campaign-product-card:not(.hide), .slide'),
            ...
            initialTouchPos = 0,
            finalTouchPos = 0,
            ballonsContainer = document.querySelector('.ballons-container'),
            slideText = ballonsContainer ? ballonsContainer.getAttribute('data-slide-number-text') : '',
            currentSlideText = ballonsContainer ? ballonsContainer.getAttribute('data-current-slide-text') : '';

        function _addAriaLabelsToSlides() {
            var slidesLength = visibleSlides.length,
                slideNumber;

            visibleSlides.forEach(function (slide, index) {
                slideNumber = index + 1;

                slide.setAttribute('aria-label', slideNumber + ' of ' + slidesLength + ' slides');
            });
        }
        ...
        function switchCurrentSlideSpanForBallon(ballonSelected) {
            if (carouselBallonsBtns) {
                carouselBallonsBtns.forEach(function (ballon) {
                    var slideNumber = Number(ballon.getAttribute('data-slide-position')) + 1,
                        ariaLabel = slideText + ' ' + slideNumber;

                    ballon.setAttribute('aria-label', ariaLabel);

                    if (ballon === ballonSelected) {
                        ballonSelected.setAttribute('aria-label', ariaLabel + ' ' + currentSlideText);
                    }
                });
            }
        }
            ...
            if (carouselBallonsBtns) {
                carouselBallonsBtns.forEach(function (ballon, index) {
                    ...
                    if ((Math.abs(currentPosition) / divider) === parseInt(ballon.getAttribute('data-slide-position'), 10)) {
                        ...
                        switchCurrentSlideSpanForBallon(ballon);
                    }
                });
            }
        ...
        function _activateSlideFromBallon(e) {
            ...
            switchCurrentSlideSpanForBallon(thisBallon);
        ...
        function _createBallons() {
            ...
            if (carouselBallons) {
                d = document;
                ulBallonsContainer = carouselBallons.querySelector('.ballons-container');

                carouselBallons.classList.remove('hide');
                visibleSlides.forEach(function (slide, index) {
                    var li = d.createElement('li'),
                        button = d.createElement('button'),
                        slideNumber = Number(slide.getAttribute('data-slide-position')) + 1,
                        ariaLabel = slideText + ' ' + slideNumber;
                    ...
                    if (index === 0) {
                        button.setAttribute('aria-label', ariaLabel + ' ' + currentSlideText);
                        button.classList.add('active');
                    }

                    li.appendChild(button);
                    ulBallonsContainer.appendChild(li);
                });
            }
