var segmentedProgressBarJS = (function () {
    ...
    function _initialize(element) {
        var d = document,
            ...
            nextBtn = form ? form.querySelector('button.button.next-btn') : null,
            backBtn = form ? form.querySelector('a.back-btn') : null,
            ...
            if (dataSegments && dataCurrentStep) {
                ...
                setTimeout(function () {
                    ...
                    levels.forEach(function (level, index) {
                        progressItems[index].classList.remove('selected');
        ...
        function _attachEvents() {
            EDC.utils.attachEvents(nextBtn, 'click', _selectProgressItem);

            if (backBtn) {
                EDC.utils.attachEvents(backBtn, 'click', _selectProgressItem);
            }
        }

        if (dataSegments && dataCurrentStep) {
            _renderSteps(dataSegments);
        } else if (nextBtn && levelsCount && levelsCount > 0) {
            _attachEvents();
