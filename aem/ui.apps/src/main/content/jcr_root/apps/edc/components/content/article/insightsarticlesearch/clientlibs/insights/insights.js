var insightsSearchJS = (function () {
    'use strict';

    // Initialize the component
    function _initialize(element) {
        var filtersBody = document.querySelector('body'),
            filterContainer = element.querySelector('.insights-search-filters'),
            filterCategories = filterContainer.querySelectorAll('.insights-search-filter'),
            ...
            initialArticles = element.querySelectorAll('.insights-search-article'),
            filters = element.querySelectorAll('.insights-search-filter'),
            openLink = element.querySelector('.insights-search-info').getAttribute('data-open-link'),
            keepScrolling = true,
            pageNum = 1,
            modalSize = 0,
            timerId = null,
        ...
        function _trackEvent(e) {
            var target = e ? e.target : '',
                ...
                obj = {
                    eventInfo: {
                        ...
                        eventText: target ? target.closest('.insights-search-article').getAttribute('data-page-name').toLowerCase() : noMatch.getAttribute('data-no-results'),
            ...
            for (i = 0; i < filters.length; i++) {
                pindex = i === 0 ? '' : i + 1;
                obj.eventInfo['eventValue' + pindex] = _getFilterValues(filters[i]);
            }
        ...
        function _renderArticles(articles) {
            ...
            if (articles.length) {
                ...
                articles.forEach(function (article) {
                    ...
                    card.classList.add('insights-search-article');
                ...
                pageArticles = articlePage.querySelectorAll('.insights-search-article');
        ...
        function _loadArticles() {
            ...
            EDC.components.InsightsSearch.getFilteredData(pageNum, selectedFilters, function (articles) {
                if (articles.length) {
        ...
        function _loadNextPage() {
            ...
            if (articlesList.clientHeight - 300 < lastKnownScrollPosition && EDC.components.InsightsSearch) {
        ...
        function _loadFirstPage() {
            ...
            if (EDC.components.InsightsSearch) {
                pageNum = 0;
                _loadArticles();
            }
        }
        ...
        function _expandCollapseFilter(e) {
            var target = e.target.classList.contains('chevron') ? e.target : e.target.parentElement,
                filterSection = target.closest('.insights-search-filter'),
        ...
        function _showMoreOptions(e) {
            var target = e.target.classList.contains('show-more') ? e.target : e.target.parentElement,
                ...
                filterSection = e.target.closest('.insights-search-filter'),
        ...
        function _attachEvents() {
            var filterBtn = element.querySelector('.show-filters'),
                ...
                articleLinks = element.querySelectorAll('.insights-search-article .title, .insights-search-article .card-img-anchor');
        ...
        EDC.components.InsightsSearch.initialize();
        isDevice = _isDevice();
        _showHideFilter();
        _sideBarInit();
    ...
    // Public methods
    function init() {
        var insightsSearch = document.querySelectorAll('.c-insights-search-filter:not([data-component-state="initialized"])');

        if (insightsSearch) {
            insightsSearch.forEach(function (elem) {
                _initialize(elem);
...
document.addEventListener('DOMContentLoaded', insightsSearchJS.init);