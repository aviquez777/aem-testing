...
public class ArticlePageHelper {
    ...
    public void getPropertiesFromPage(SlingHttpServletRequest request, Page page) {
        if (page != null) {
            // Initialize variable
            String tagId = null;
            List<String> pageTags = null;
            // Create a list array containing all the page tags ids
            pageTags = Arrays.stream(page.getTags()).map(Tag::getTagID).collect(Collectors.toList());
            ...
            for (ArticleContentType contentType : ArticleContentType.values()) {
                ...
                if (pageTags != null && pageTags.contains(tagId)) {
            ...
            if (pageTags != null && pageTags.contains(Constants.TAGS_EDC_PREMIUM)) {
                this.isPremium = true;
            }
            // Set the title
    ...
    /**
     * Return the content type CSS class name of this article page (e.g., blog,
     * article, etc.).
     *
    ...
    /**
     * Return the language-appropriate content type title of this article page
     * (e.g., Blog, Article, etc.).
     *
    ...
    /**
     * Returns the correct sort date for the given article page. The correct sort
     * date is either first published, last published or default date depending on
     * the status of this page and the setting in the Article Hero component.
     * <p>
     * If the article's sort date type is "first published" or "no date" and there
     * is a "first published date", that date is returned.
     * <p>
     * If the article's sort date type is "last published" and there is a "last
     * published date", that date is returned.
     * <p>
     ...
     * @param defaultDate Default date to be returned if article page date can not
    ...
    public static Calendar getArticlePageSortDate(Page page, ValueMap valueMap, Calendar defaultDate) {
        ...
        if (valueMap.containsKey(Constants.Properties.PUBLISH_DATE)) {
            ...
            if (sortDateType != null) {
                ...
                // -------------------------------------------------------------
                if ((sortDateType.equals(Constants.Properties.FIRST))
                        || (sortDateType.equals(Constants.Properties.NO_DATE))) {
                    // ---------------------------------------------------------
        ...
        if (log.isDebugEnabled()) {
            ...
            log.debug("publishDate: {} pageDate: {}",  sortDateType,
                     (pageDate != null ? formatter.format(pageDate.getTime()) : null));
        }
        return pageDate;
    }
    ...
    public static class ListSort implements Comparator<Page>, Serializable {
         ...
         * @return A negative integer, zero, or a positive integer as the first page
         *         (page1) is less than, equal to, or greater than the second (page2),
         *         respectively.
         */
        @Override
        public int compare(Page page1, Page page2) {
            ...
            // ------------------------------------------------------------------
            Calendar page1Date = ArticlePageHelper.getArticlePageSortDate(page1, page1ValueMap,
                    page1ValueMap.get(Constants.Properties.JCR_CREATED, Calendar.class));
            Calendar page2Date = ArticlePageHelper.getArticlePageSortDate(page2, page2ValueMap,
                    page2ValueMap.get(Constants.Properties.JCR_CREATED, Calendar.class));
            // ------------------------------------------------------------------
